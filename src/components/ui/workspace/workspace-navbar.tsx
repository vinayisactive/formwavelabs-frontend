...
const WorkspaceNavbar : FC<WorkspaceNavbarProps> = ({workspaceName, userRole, members, setIsSidebarOpen, setInviteModalOpen, setCreateFormModal, wsId}) => {
    ...
    <div className="flex justify-between items-center w-full">
      ...
      <div className="flex justify-end items-center gap-1 ">
        {(userRole === "ADMIN" || userRole === "OWNER") && (
          <button className="border px-3 py-1 rounded-md shadow-sm text-black text-sm hover:bg-black hover:text-white flex items-center gap-1 whitespace-nowrap"
        ...
        <button
          className="group flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-gray-100 transition-colors relative"
        ...
        >
          ...
          {isMembersModalOpen && (
            <WorkspaceMembers userRole={userRole} members={members} wsId={wsId} />
          )}
        </button>

        {(userRole === "ADMIN" || userRole === "OWNER") && (
          <button className="flex items-center gap-1 px-2 py-1 rounded-md hover:bg-gray-200 text-sm" onClick={() => setInviteModalOpen((prev) => !prev)}>
            <UserPlus size={15} />
